...
def getRange(index):
	if index == cores-1:
		diff = float(specimen.shape[0])/cores
		return(int(index*diff),specimen.shape[0])
	elif index < cores-1:
		diff = float(specimen.shape[0])/cores
		return(int(index*diff),int((index+1)*diff))
	else:
		print("More processes than cores")

def propagateToSensor(image,image_distance,index,output): # Main function for getting the images
		...
		for y in range(image.shape[1]):
			...
			for x1 in range(center_x-radius,center_x+radius): # Within that circle
				...
				for y1 in range(center_y-extent,center_y+extent):
					...
					sensor_flat[sensor_x*para['sensor_xpixels']+sensor_y] = sensor_flat[sensor_x*para['sensor_xpixels']+sensor_y] + image[x][y]*normalize
					#Changing the shared sensor raw array
...
output = mp.SimpleQueue() # Output class in the multiprocessing library
...
if __name__ == "__main__": # Prevention from spawning infinite processes
	...
	for index in range(3,4): # 2,3 arbitrary. Can range between the number of slices
		...
		if para['x_tilt'] == 1 and para['y_tilt'] == 1:
			processes = [mp.Process(target=propagateToSensor, args=(section,image_distance,x,output)) for x in range(cores)]
			# Target function, Arguments to be passed in, Number of processes
		else:
			processes = [mp.Process(target=propagateToSensorAngle, args=(section,image_distance,x,output)) for x in range(cores)]
		for p in processes: # Start processes
			p.start()
		for p in processes: # End processes
			p.join()
